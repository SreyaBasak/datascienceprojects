...
Drop view Region_extended_pretty;
 ...
 SELECT `Region-zone` as Regions , `Hub Name` , FHRID, `DeliveryType`,
coalesce(sum((case when DateOfRSPS = "01-May-20" then delivered end)), ' ') as "01-May-20",
    coalesce(sum((case when DateOfRSPS = "02-May-20" then delivered end)), ' ') as "02-May-20",
    coalesce(sum((case when DateOfRSPS = "03-May-20" then delivered end)), ' ') as "03-May-20",
    coalesce(sum((case when DateOfRSPS = "04-May-20" then delivered end)), ' ') as "04-May-20",
    coalesce(sum((case when DateOfRSPS = "05-May-20" then delivered end)), ' ') as "05-May-20"
  from region_date_delivered
  Group by FHRID
